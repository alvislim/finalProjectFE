...
function Dashboard() {
  ...
  const onSubmit = async (data, e) => {
    try {
      ...
      const response = await axios.post(`${url}/coldstorage`, {
        ...data,
        email,
      });
      ...
      if (response.data.success) {
        ...
        setIsLoading(false);
        seterrorPost(
          "We aplogized, we are currently having some issues with our server"
        );
          ...
          <Container
            ...
            <Card body>
              <Form onSubmit={handleSubmit(onSubmit)}>
                ...
                <Button input type='submit' variant='dark'>
                  Submit
                </Button>
              </Form>
            </Card>
          ...
          <div>
            {userItem === 0 ? (
              <Container
                ...
                style={{ height: "100vh" }}>
                <Card.Subtitle>
                  Please click on the above Button to start TRACKING now!
                </Card.Subtitle>
              </Container>
            ) : (
